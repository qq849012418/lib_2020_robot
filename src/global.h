...
void *core_sysdep_network_init(void)
{
    ...
    handle = (core_network_handle_t *)malloc(sizeof(core_network_handle_t));
    if (handle == NULL) {
        return NULL;
    }
...
int32_t core_sysdep_network_setopt(void *handle, core_sysdep_network_option_t option, void *data)
{
    ...
    switch (option) {
        ...
        case CORE_SYSDEP_NETWORK_HOST: {
            network_handle->host = (char*)malloc(strlen((const char*)data) + 1);
            if (network_handle->host == NULL) {
                printf("malloc failed\n");
                return STATE_PORT_MALLOC_FAILED;
            }
            memset(network_handle->host, 0, strlen((const char*)data) + 1);
            memcpy(network_handle->host, data, strlen((const char*)data));
        }
        break;
        case CORE_SYSDEP_NETWORK_BACKUP_IP: {
            memcpy(network_handle->backup_ip, data, strlen((const char*)data));
        }
        break;
        ...
        case CORE_SYSDEP_NETWORK_CRED: {
            network_handle->cred = (aiot_sysdep_network_cred_t*)malloc(sizeof(aiot_sysdep_network_cred_t));
        ...
        case CORE_SYSDEP_NETWORK_PSK: {
            ...
            network_handle->psk.psk_id = (char*)malloc(strlen(psk->psk_id) + 1);
            ...
            network_handle->psk.psk =  (char*)malloc(strlen(psk->psk) + 1);
